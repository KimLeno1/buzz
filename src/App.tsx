...
const App: React.FC = () => {
  ...
  const features = [
    ...
    },
    {
      ...
      icon: <LayoutGrid className="text-[#004F71]" size={32} />
    }
  ];
  ...
  const handleCheckerPurchase = (checker: any, phone: string, paymentMethod: 'paystack' | 'wallet') => {
    setIsProcessing(true);
    setLastPurchase({ 
      bundle: { title: checker.name, price: checker.price }, 
      beneficiary: phone, 
      sender: user?.phone || user?.email || 'Guest',
      paymentMethod: paymentMethod === 'wallet' ? 'Account Balance' : 'Paystack'
    });
    ...
    setTimeout(() => {
      ...
      if (paymentMethod === 'wallet' && user) {
        setUser({
          ...user,
          balance: user.balance - checker.price
        });
      }
      ...
      const newTx: WalletTransaction = {
        ...
        date: new Date().toLocaleDateString() + ' ' + new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        status: 'Completed',
        description: `Purchased ${checker.name} for ${phone} (${paymentMethod})`
      };
      setTransactions(prev => [newTx, ...prev]);
    }, 1500);
  };
  ...
  const handleAuthSuccess = (userData: { name: string; email: string; phone?: string; isAdmin?: boolean }) => {
    setUser({
      id: `usr-${Math.floor(Math.random() * 1000)}`,
      name: userData.name,
      email: userData.email,
      phone: userData.phone || '0553877937',
      rank: 'Seeker',
      xp: 0,
      balance: 100,
  ...
  if (isAdminPanelOpen && user?.isAdmin) {
    return (
      <AdminPanel 
        isOpen={true}
    ...
    >
        ...
        <>
          ...
          <section className="relative pt-20 pb-28 bg-white border-b border-gray-100">
            <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#004F71]/10 text-[#004F71] text-[10px] font-bold uppercase tracking-wider rounded-full mb-8">
                ...
                <h1 className="text-4xl md:text-6xl font-extrabold text-gray-900 tracking-tight leading-tight mb-6">
                  ...
                  <span className="text-[#004F71]">Premium Data Bundles.</span>
                </h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <button 
                    ...
                    className="px-8 py-4 bg-[#004F71] text-[#FFCC00] font-bold rounded-xl shadow-lg hover:bg-[#003d57] transition-all flex items-center justify-center gap-2"
                  >
                   ...
                   <div className="space-y-4">
                     {[
                       ...
                       <div key={i} className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
                         <div className="w-10 h-10 bg-[#004F71]/10 rounded-xl flex items-center justify-center text-[#004F71]">
                            <Wifi size={20} />
                         </div>
                         <div className="flex-1">
                            ...
                            <div className="h-1.5 w-full bg-gray-50 rounded overflow-hidden">
                               <div className="h-full bg-[#004F71] rounded" style={{ width: `${Number(net.speed) * 2}%` }} />
                            </div>
                         </div>
                         <div className="text-xs font-bold text-[#004F71]">{net.speed}MB/s</div>
                       </div>
                     ))}
                   </div>
          ...
          {!user && (
            <section className="py-12 bg-[#004F71] relative overflow-hidden">
                ...
                <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[2.5rem] p-8 md:p-12 flex flex-col lg:flex-row items-center justify-between gap-12">
                  <div className="max-w-xl">
                    ...
                    <h2 className="text-3xl md:text-4xl font-black text-white mb-6 tracking-tight">
                      ...
                      <span className="text-[#FFCC00]">Better rates, faster top-ups.</span>
                    </h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-gray-100 text-xs font-medium opacity-80">Exclusive member-only pricing on selected bundles.</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-4">
                        <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center text-white shrink-0">
                        ...
                        <div>
                          ...
                          <p className="text-gray-100 text-xs font-medium opacity-80">Earn XP and rank up for even bigger discounts.</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-4">
                        <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center text-white shrink-0">
                        ...
                        <div>
                          ...
                          <p className="text-gray-100 text-xs font-medium opacity-80">Track all your purchases and reuse numbers easily.</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-4">
                        <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center text-white shrink-0">
                        ...
                        <div>
                          ...
                          <p className="text-gray-100 text-xs font-medium opacity-80">Get help faster with our dedicated member support.</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="w-full lg:w-auto">
                    <div className="bg-white rounded-3xl p-8 shadow-2xl text-center lg:w-80">
                      <div className="w-16 h-16 bg-[#004F71]/10 text-[#004F71] rounded-2xl flex items-center justify-center mx-auto mb-6">
                        <UserIcon size={32} />
                      </div>
                      ...
                      <button 
                        ...
                        className="w-full py-4 bg-[#004F71] text-[#FFCC00] font-black rounded-xl hover:bg-[#003d57] transition-all shadow-lg active:scale-95"
                      >
                        Create Account
                      </button>
      ...
      <section id="bundles-section" className="px-6 md:px-12 py-20 max-w-7xl mx-auto border-b border-gray-100 scroll-mt-20">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
          <div>
            <h2 className="text-3xl font-black text-[#004F71] tracking-tight mb-2">Available <span className="text-[#FFCC00] bg-[#004F71] px-2 rounded-lg">Bundles</span></h2>
            ...
            {categories.map(cat => (
              <button 
                ...
                className={`px-4 py-2 text-xs font-black rounded-xl border transition-all ${
                  activeCategory === cat 
                  ? 'bg-[#004F71] text-[#FFCC00] border-[#004F71] shadow-md' 
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col items-center">
            <button 
              ...
              className="px-10 py-5 bg-[#004F71] text-[#FFCC00] font-extrabold rounded-2xl shadow-xl hover:bg-[#003d57] hover:-translate-y-1 transition-all active:scale-95 flex items-center gap-3 text-lg"
            >
              Get Started Now
              <ArrowRight size={24} />
            </button>
      ...
      {/* Admin Panel (Standalone handled above, but keeping for safety if needed as modal elsewhere) */}
      {/* Auth Modal */}
      <AuthModal 
      ...
      {isProjectDashboardOpen && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 md:p-10">
          <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-md" onClick={() => setIsProjectDashboardOpen(false)} />
          <div className="relative bg-gray-50 w-full max-w-6xl h-full max-h-[90vh] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
            <div className="p-8 bg-white border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-2xl font-black text-gray-900">Project Dashboard</h2>
              <button onClick={() => setIsProjectDashboardOpen(false)} className="p-3 hover:bg-gray-100 rounded-2xl transition-colors">
                <X size={24} />
              </button>
            </div>
      ...
      {isChatOpen && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 md:p-10">
          <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-md" onClick={() => setIsChatOpen(false)} />
          <div className="relative w-full max-w-4xl h-full max-h-[85vh] animate-in zoom-in-95 duration-300">
            <ChatInterface />
            <button 
          ...
          <div className="relative bg-white w-full max-w-lg rounded-3xl p-10 text-center shadow-2xl animate-in zoom-in-95 duration-300">
             ...
             <div className="bg-gray-50 rounded-2xl p-6 mb-10 text-left border border-gray-100">
                <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-200/50">
                   ...
                   <div className="flex items-center gap-2 text-[#004F71] font-bold text-sm">
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-bold text-[#004F71]">GH₵ {lastPurchase?.bundle?.price?.toLocaleString()}</span>
                  </div>
                </div>
                ...
                {cart.length === 0 ? (
                  ...
                  cart.map(item => (
                    <div key={item.id} className="flex gap-4 p-4 bg-white border border-gray-100 rounded-xl shadow-sm">
                      <div className="w-12 h-12 bg-gray-50 rounded-lg flex items-center justify-center font-bold text-[#004F71] text-xs">{item.category}</div>
                      <div className="flex-1">
             ...
             {cart.length > 0 && (
               <div className="pt-8 border-t border-gray-100 space-y-6">
                   ...
                   <div className="space-y-1">
                     ...
                     <div className="relative">
                       ...
                       <input
                         ...
                         className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#004F71] outline-none"
                       />
                     </div>
                   </div>
                   <div className="space-y-1">
                 ...
                 <div className="space-y-2">
                   ...
                   <div className="grid grid-cols-2 gap-2">
                      <button 
                        ...
                        className={`flex flex-col items-center gap-1.5 p-4 rounded-xl border-2 transition-all ${cartPaymentMethod === 'paystack' ? 'border-[#004F71] bg-[#004F71]/5 shadow-sm' : 'border-gray-100 bg-white'}`}
                      >
                        <CreditCard size={20} className={cartPaymentMethod === 'paystack' ? 'text-[#004F71]' : 'text-gray-400'} />
                        <span className={`text-[10px] font-black ${cartPaymentMethod === 'paystack' ? 'text-[#004F71]' : 'text-gray-400'}`}>Paystack</span>
                      </button>
                      <button 
                        onClick={() => setCartPaymentMethod('wallet')}
                        className={`flex flex-col items-center gap-1.5 p-4 rounded-xl border-2 transition-all ${cartPaymentMethod === 'wallet' ? 'border-[#004F71] bg-[#004F71]/5 shadow-sm' : 'border-gray-100 bg-white'} ${!user ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
                      >
                        <Wallet size={20} className={cartPaymentMethod === 'wallet' ? 'text-[#004F71]' : 'text-gray-400'} />
                        <span className={`text-[10px] font-black ${cartPaymentMethod === 'wallet' ? 'text-[#004F71]' : 'text-gray-400'}`}>Balance</span>
                      </button>
                   </div>
                 </div>
                 ...
                 <button 
                  ...
                  disabled={isProcessing}
                  className={`w-full py-4 text-white font-bold rounded-xl shadow-lg transition-all flex items-center justify-center gap-3 disabled:bg-gray-400 ${cartPaymentMethod === 'wallet' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-[#004F71] hover:bg-[#003d57]'}`}
                 >
                   {isProcessing ? (
                     <>
