...
const ProfilePanel: React.FC<ProfilePanelProps> = ({ isOpen, onClose, user, onLogout, onOpenTopUp, onOpenTransfer, onViewTransactions, onUpdateUser }) => {
  ...
  const RANKS = [
    ...
    { name: 'Oracle', minXp: 230, color: 'bg-[#004F71]' },
      ...
      <div className="relative bg-white w-full max-w-2xl h-full sm:h-auto sm:max-h-[90vh] sm:rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col">
        ...
        <div className="relative h-48 bg-slate-900 flex items-center px-8 sm:px-12 overflow-hidden shrink-0">
          <div className="absolute inset-0 opacity-20">
            <div className="absolute top-0 left-0 w-64 h-64 bg-[#004F71]/20 rounded-full -translate-x-1/2 -translate-y-1/2 blur-3xl" />
          ...
          <div className="relative z-10 flex items-center gap-6 w-full">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[#FFCC00] text-[10px] font-black uppercase tracking-widest">{user.rank}</span>
          ...
          {isEditing ? (
            <form onSubmit={handleUpdateSubmit} className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#004F71] transition-colors" size={18} />
                    <input
                      type="text"
                      name="name"
                      required
                      ...
                      className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-[#004F71]/10 focus:border-[#004F71] outline-none transition-all"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#004F71] transition-colors" size={18} />
                    <input
                      type="email"
                      name="email"
                      required
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#004F71] transition-colors" size={18} />
                    <input
                      type="password"
                      name="password"
                      placeholder="••••••••"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Shield className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#004F71] transition-colors" size={18} />
                    <input
                      type="password"
                      name="newPassword"
              ...
              <button
                type="submit"
                disabled={isSaving}
                className="w-full py-5 bg-[#004F71] text-white font-black rounded-3xl shadow-xl hover:bg-[#003d57] transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50"
              >
                {isSaving ? (
            ...
            <>
              {/* Wallet Section */}
              <div className="space-y-4">
                <div className="bg-[#004F71]/5 p-6 rounded-3xl border border-[#004F71]/10 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-[#004F71] shadow-sm">
                      <Wallet size={24} />
                    </div>
                    <div>
                      <div className="text-[10px] font-black text-[#004F71]/60 uppercase tracking-widest mb-0.5">Wallet Balance</div>
                      <div className="text-xl font-black text-slate-900">GH₵ {user.balance.toLocaleString()}</div>
                    </div>
                  </div>
                  <button 
                    onClick={onViewTransactions}
                    className="p-3 bg-white text-slate-400 rounded-xl hover:text-[#004F71] transition-colors shadow-sm"
                  >
                    <History size={20} />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button 
                    onClick={onOpenTopUp}
                    className="bg-[#004F71] text-white p-6 rounded-3xl flex flex-col gap-4 group hover:bg-[#003d57] transition-all shadow-lg"
                  >
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                <div className="flex justify-between items-end">
                  ...
                  <div className="text-right">
                    <span className="text-xs font-black text-[#004F71]">{Math.round(progress)}%</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  <Award size={12} className="text-[#004F71]" />
