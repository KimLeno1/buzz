      ...
      <div className="relative bg-white w-full max-w-xl h-full shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col">
        ...
        <div className="p-8 border-b border-slate-100 bg-white sticky top-0 z-20 shrink-0">
          ...
          <div className="flex items-center gap-2 px-1">
             <div className={`h-1.5 rounded-full transition-all duration-300 ${step === 'plans' ? 'w-8 bg-[#004F71]' : 'w-2 bg-[#004F71]/10'}`} />
             <div className={`h-1.5 rounded-full transition-all duration-300 ${step === 'details' ? 'w-8 bg-[#004F71]' : 'w-2 bg-[#004F71]/10'}`} />
             <div className={`h-1.5 rounded-full transition-all duration-300 ${step === 'contact' ? 'w-8 bg-[#004F71]' : 'w-2 bg-[#004F71]/10'}`} />
          </div>
        </div>
          ...
          {step === 'plans' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              {plans.map((plan) => (
                ...
                >
                  {plan.id === 'complex' && (
                    <div className="absolute top-6 right-6 px-3 py-1 bg-[#FFCC00] text-black text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg z-10">
                      Recommended
                    </div>
                  )}
                  ...
                  <div className="flex items-start gap-6 mb-8">
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center shrink-0 ${
                      plan.id === 'simple' ? 'bg-[#004F71]/5 text-[#004F71]' : 'bg-[#FFCC00]/10 text-[#004F71]'
                    }`}>
                  ...
                  <div className="space-y-3 mb-8">
                    {plan.features.slice(0, 4).map((feature, i) => (
                      <div key={i} className="flex items-center gap-3 text-sm font-bold text-slate-600">
                        <CheckCircle2 size={16} className={`shrink-0 ${plan.id === 'simple' ? 'text-[#004F71]' : 'text-[#FFCC00]'}`} />
                        {feature}
                      </div>
                    ))}
                  </div>
                  ...
                  <button className={`w-full py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg ${
                      plan.id === 'simple' ? 'bg-[#004F71] text-white' : 'bg-[#004F71] text-[#FFCC00]'
                    }`}>
            ...
            <form onSubmit={handleDetailsNext} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                <div className="flex items-center gap-3 mb-2">
                  <div className="p-2 bg-[#004F71]/5 text-[#004F71] rounded-lg">
                    <FileText size={20} />
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-[#004F71]/10 focus:border-[#004F71] outline-none transition-all"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-[#004F71]/10 focus:border-[#004F71] outline-none transition-all resize-none"
                  />
                </div>
              ...
              <button 
                type="submit"
                className="w-full py-5 bg-[#004F71] text-white font-black rounded-2xl shadow-xl hover:bg-[#003d57] transition-all flex items-center justify-center gap-2 active:scale-95"
              >
          ...
          {step === 'contact' && (
            <form onSubmit={handleSubmitOrder} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <button 
                ...
                className="w-full py-5 bg-[#004F71] text-white font-black rounded-2xl shadow-xl hover:bg-[#003d57] transition-all flex items-center justify-center gap-3 active:scale-95 disabled:bg-slate-300"
              >
                {isProcessing ? (
                  <>
