...
const ServiceCard: React.FC<ServiceCardProps> = ({ title, description, icon, onClick, color, badge }) => (
  <div 
    onClick={onClick}
    className="group relative bg-white rounded-3xl border border-gray-100 p-8 shadow-sm hover:shadow-2xl hover:border-[#004F71]/20 transition-all duration-500 cursor-pointer overflow-hidden flex flex-col h-full active:scale-[0.98]"
  >
    ...
    <div className="relative z-10">
      ...
      {badge && (
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-[#004F71]/10 text-[#004F71] text-[10px] font-black uppercase tracking-widest rounded-full mb-4">
          <Sparkles size={10} /> {badge}
        </div>
      )}

      <h3 className="text-xl font-black text-gray-900 mb-3 group-hover:text-[#004F71] transition-colors">
        {title}
      </h3>
      ...
      <div className="flex items-center gap-2 text-sm font-black text-[#004F71] group/link">
        Get Started 
    ...
    <section id="services-section" className="px-6 md:px-12 py-24 max-w-7xl mx-auto border-b border-gray-100 scroll-mt-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
        <div className="max-w-xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#FFCC00]/10 text-[#004F71] text-[10px] font-black uppercase tracking-[0.2em] rounded-full mb-6">
            <Sparkles size={12} fill="currentColor" /> Premium Digital Services
          </div>
          <h2 className="text-4xl font-black text-[#004F71] tracking-tight mb-4">
            More than just <span className="text-[#FFCC00] bg-[#004F71] px-2 rounded-lg">Mobile Data.</span>
          </h2>
        ...
        <button 
          className="group flex items-center gap-3 text-sm font-black text-gray-400 hover:text-[#004F71] transition-all uppercase tracking-widest"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <ServiceCard
          ...
          description="Monitor the progress of your design orders, download assets, and request revisions."
          icon={<Layers className="text-[#004F71]" />}
        ...
        <ServiceCard
          title="Live Support"
          description="Connect with our dedicated support team for real-time assistance with your orders."
