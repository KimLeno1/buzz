...
const NotificationPanel: React.FC<NotificationPanelProps> = ({ isOpen, onClose }) => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'promo': return <Zap className="text-[#004F71]" size={20} fill="currentColor" />;
      ...
      <div className="relative bg-white w-full max-w-xl h-full shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col">
        ...
        <div className="p-8 border-b border-slate-100 shrink-0 bg-white sticky top-0 z-10">
          <div className="flex justify-between items-start mb-6">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#004F71]/5 text-[#004F71] text-[10px] font-black uppercase tracking-widest rounded-full mb-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <button 
              onClick={markAllRead}
              className="text-[10px] font-black text-[#004F71] uppercase tracking-widest hover:text-[#003d57] transition-colors flex items-center gap-1.5"
            >
            ...
            notifications.map((notif) => (
              <div 
                ...
                className={`group relative bg-white p-6 rounded-[2rem] border transition-all duration-300 ${
                  notif.isRead ? 'border-slate-100 opacity-70' : 'border-[#004F71]/20 shadow-md ring-1 ring-[#004F71]/5'
                }`}
              >
                {!notif.isRead && (
                  <div className="absolute top-6 right-6 w-2 h-2 bg-[#004F71] rounded-full animate-pulse shadow-[0_0_10px_rgba(0,79,113,0.5)]" />
                )}
        ...
        <div className="p-8 border-t border-slate-100 bg-white space-y-4">
           <div className="bg-slate-50 p-4 rounded-2xl flex items-start gap-3 border border-slate-100">
              <ShieldCheck className="text-[#004F71] shrink-0" size={18} />
